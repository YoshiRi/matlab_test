...
%% X obs base original éxîzã…ÇÃï€éù

% choose pole
p_hz = -3*pi*2;
%c_pole = [p_hz,p_hz/sqrt(2)+p_hz/sqrt(2)*1i,p_hz/sqrt(2)-p_hz/sqrt(2)*1i];
c_pole = [p_hz,p_hz/2 + p_hz/2*sqrt(3)*1i,p_hz/2 - p_hz/2*sqrt(3)*1i];
...
%         Kpp = place(A(2:3,2:3)',A(2:3,2:3)'*[H2(2:3)]',pole(2:3))';
        Kpp = place(A(2:3,2:3)',A(2:3,2:3)'*[H2(2:3)]',pole2)';
        Kgain2 = [0;Kpp];
...
%% X obs base original éxîzã…ÇÃîÒï€éù
...
%% X obs base original éxîzã…ÇÃîÒï€éù ÇªÇÃÇQ
